...
class CreateEnlaceTable extends Migration
{
    ...
    public function up()
    {
        Schema::create($this->tableName, function (Blueprint $table) {
            ...
            $table->integer('ativos_id1')->unsigned();
            $table->integer('ativos_id2')->unsigned();
            ...
            $table->index(["ativos_id1"], 'fk_enlace_ativos1_idx');
            $table->index(["ativos_id2"], 'fk_enlace_ativos2_idx');

            $table->foreign('ativos_id1', 'fk_enlace_ativos1_idx')
                ->references('id')->on('ativos')
                ->onDelete('no action')
                ->onUpdate('no action');
            $table->foreign('ativos_id2', 'fk_enlace_ativos2_idx')
                ->references('id')->on('ativos')
                ->onDelete('no action')
                ->onUpdate('no action');
        });
